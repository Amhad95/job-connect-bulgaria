            ...
            <div className="min-h-[calc(100vh-4rem)] bg-background flex border-t">
                ...
                <aside className={`bg-card border-r w-64 flex-shrink-0 transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 absolute md:static h-full min-h-[calc(100vh-4rem)] z-10`}>
                    <div className="p-4 border-b flex justify-between items-center md:hidden">
                        <h2 className="text-lg font-bold text-foreground">{t("dashboard.title", "Applicant Dashboard")}</h2>
                        <button className="text-muted-foreground" onClick={() => setSidebarOpen(false)}>×</button>
                    </div>
                ...
                <main className="flex-1 flex flex-col relative overflow-hidden">
                    <header className="md:hidden bg-card border-b h-14 flex items-center px-4 shrink-0">
                        <button className="p-2 -ml-2 text-muted-foreground flex items-center" onClick={() => setSidebarOpen(true)}>
                            <Menu className="w-6 h-6 inline-block" />
                        </button>
                        <span className="ml-3 font-semibold text-foreground">{t("dashboard.title", "Applicant Dashboard")}</span>
                    </header>

                    <div className="p-4 md:p-8 flex-1 overflow-auto bg-muted/20">
                        <Outlet />
                    </div>
                </main>
            </div>
