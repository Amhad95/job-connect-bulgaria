...
create index idx_exp_user on applicant_experiences(user_id, sort_order);

-- B3) Experience achievement bullets
create table applicant_experience_bullets (
  id            uuid primary key default gen_random_uuid(),
...
create index idx_exp_bullets_exp on applicant_experience_bullets(experience_id, sort_order);

-- B4) Experience role-specific skills
create table applicant_experience_skills (
  id            uuid primary key default gen_random_uuid(),
  experience_id uuid not null references applicant_experiences(id) on delete cascade,
  user_id       uuid not null references auth.users(id) on delete cascade,
...
create index idx_exp_skills_exp on applicant_experience_skills(experience_id);
...
create index idx_edu_user on applicant_education(user_id, sort_order);
...
create index idx_skills_user on applicant_skills(user_id, sort_order);
...
create index idx_certs_user on applicant_certificates(user_id, sort_order);
...
create index idx_links_user on applicant_links(user_id, sort_order);
...
create index idx_view_history_user on job_view_history(user_id, viewed_at desc);

create table job_interest_tags (
  id         uuid primary key default gen_random_uuid(),
  user_id    uuid not null references auth.users(id) on delete cascade,
...
create index idx_interest_tags_user on job_interest_tags(user_id);
...
create table application_tracker_items (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references auth.users(id) on delete cascade,
...
create index idx_tracker_user on application_tracker_items(user_id, stage);
...
-- Macro: each profile table gets SELECT/INSERT/UPDATE/DELETE for own user_id
do $$
declare
  tbl text;
  col text;
...
-- Authenticated users can insert new catalog entries (low-popularity suggestions grow organically)
...
-- =============================================================================
-- K) CANDIDATE READ OWN APPLICATIONS (ensure policy exists)
-- Already handled by existing migration, but ensure the select policy includes
-- the new snapshot columns (it does, since * is already in the policy).
-- =============================================================================

-- =============================================================================
-- L) SEED CATALOGS (minimal starter data)
-- =============================================================================
